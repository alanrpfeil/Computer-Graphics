...
bool binary_to_compressed_c ( const char* filename, const char* symbol, bool use_base85_encoding, bool use_compression )
{
	...
	//=== Modif: saving as a file ===
	char outfile[80];
	snprintf(outfile,80,"%s.cpp",symbol);
	FILE* out = fopen(outfile,"wt");
	if ( !out ) return 0;
	//FILE* out = stdout;

	// Output as Base85 encoded
	fprintf(out, "// Original file: %d bytes\n", (int)data_sz);
	fprintf(out, "// Exported using binary_to_compressed_c.cpp\n");
	const char* compressed_str = use_compression ? "compressed_" : "";
	if (use_base85_encoding)
	{
		// Modif: updated printfs:
		fprintf ( out, "# define PNG_ENCODED_SIZE %d\n", (int)((compressed_sz+3)/4)*5 );
		fprintf ( out, "static const char png_%sdata_base85[%d+1] =\n	\"", compressed_str, (int)((compressed_sz+3)/4)*5 );

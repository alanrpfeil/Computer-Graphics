	...
	{	
		...
		for (int i = 0; i < n; i++) {		//for every ring
			for (int j = 0; j <= n; j++) {		//for every point
					...
					xc = (R + (r * cos(2 * PI / n * (i + 1)))) * cos(2 * PI / n * (j + 1));	//getting next j iteration to connect to
		...
		for (int i = 0; i < n; i++) {		//for every ring
			for (int j = 0; j <= n; j++) {		//for every point
				for (int k = 0; k <= 1; k++) {		//to make every triangle
					
					double s = (i + k) % n + 0.5;	//for drawing a triangle
					double t = j % (n + 1);			//for drawing a triangle

					x = (R + (r * cos(s * 2 * PI / n))) * cos(t * 2 * PI / n);
					y = (R + (r * cos(s * 2 * PI / n))) * sin(t * 2 * PI / n);
					z = r * sin(s * 2 * PI / n);
